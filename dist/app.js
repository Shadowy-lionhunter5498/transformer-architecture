...
const icons={play:'<path d="m8 5 11 7-11 7z"/>',pause:'<path d="M9 5v14M15 5v14"/>',replay:'<path d="M4 10a8 8 0 1 1 1 8M4 4v6h6"/>'};
...
function format(n){return n===1048576?'1M':n>=1024&&n%1024===0?(n/1024)+'K':n.toLocaleString('en-US')}
function bytes(n){return n>=1e9?(n/1e9).toFixed(2)+' GB':n>=1e6?(n/1e6).toFixed(1)+' MB':Math.round(n/1000)+' KB'}
...
lines(g,[[-1.5,-1.55,.1],[-1.5,2.7,.1]],col,.25);lines(g,[[1.5,-1.55,.1],[1.5,2.7,.1]],col,.25);
 ...
 d.shell=[];v.root.traverse(o=>{if(o.userData.component_id===n.id)o.traverse(m=>{if(m.isMesh)d.shell.push(m)})});const shortNames={vision_encoder:'Vision',vision_embedding:'Projector',text_embedding:'Text',residual_input:'mHC',hidden_states:'Encoder output',engram:'Engram ×2'};const title=shortNames[n.id]??(n.id==='encoder_attention'?'Self-attention':n.id==='decoder_attention'?'Masked attention':n.id==='decoder_cross'?'Cross-attention':n.part==='ffn'&&!v.side?'Feed-forward':n.label.replaceAll('\n',' '));d.label=addLabel(v,title,[n.x,n.y,1.65],'component',()=>focus(v,n));d.metric=addLabel(v,'',[n.x,n.y+2.6,2.7],'metric '+(v.side?'modern':'original'));d.metric.opacity=0;d.label.node=n;d.label.el.dataset.component=n.id;d.metric.el.dataset.component=n.id;if(n.kind==='box'){const guide=document.createElementNS(guideSVG.namespaceURI,'path');guideSVG.append(guide);d.label.guide=guide;d.label.el.classList.add('callout');const ink='#'+(d.shell[0]?.material.color.getHexString()??n.color);d.label.el.style.setProperty('--component-colour',ink);guide.style.stroke=ink;d.label.el.onpointerenter=()=>guide.classList.add('active');d.label.el.onpointerleave=()=>guide.classList.remove('active');}d.metric.owner=d;if(!title)d.label.el.hidden=true;return d;}
...
function moveToComponents(nodes,side,animate=true){
 ...
 if(n)$('#selection-name').textContent=(n.branch?n.branch[0].toUpperCase()+n.branch.slice(1)+' · ':'')+n.label.replaceAll('\n',' ');$('#selection').hidden=!n;
}
...
function tickInterior(v,d){const {n,g}=d,col=v.side?palette.new:palette.old,pos=new THREE.Vector3(n.x,n.y,.7),distance=v.camera.position.distanceTo(pos),pixels=Math.max(n.w,2.6)*height/(2*Math.tan(v.camera.fov*Math.PI/360)*distance);const weight=story.active?(v.storyWeights?.get(n.id)??0):0,raw=story.active?weight:v.selected?.id===n.id?1:smooth(112,235,pixels),hot=v.hot===d;d.zoom=THREE.MathUtils.lerp(d.zoom,story.active?raw:hot?raw:0,.10);const zoom=d.zoom,otherFade=story.active?1-(v.storyFocus??0)*(1-weight)*.985:v.hot&&!hot?1-v.hot.zoom*.87:1;d.opacity=otherFade;
 ...
 if(d.cells){
  ...
  if(v.side){
   ...
   d.kvAnnotation.el.textContent=spec.hasGlobal?(spec.mode==='Full'?'New KV':'Shared KV'):'';d.indexAnnotation.el.textContent=spec.hasIndexer?'32 index heads':spec.hasGlobal?'Reused indices':'';d.indexAnnotation.localAnchor.set(...(spec.hasIndexer?[2.78,-.69,0]:[0,-2.0,0]));
   d.metric.el.innerHTML=`64 Q heads<small>${spec.mode} · ${spec.selected} global entries + ${spec.local} local tokens</small>`;
  }else d.metric.el.innerHTML=`8 heads<small>${n.part==='cross'?'encoder K/V · decoder Q':n.branch==='decoder'?'masked self-attention':'bidirectional self-attention'}</small>`;

 }else if(d.experts){const active=Array.from({length:6},(_,i)=>(step*53+i*61+17)%384);for(let i=0;i<(v.side?385:32);i++){const shared=i===384,yes=!v.side||shared||active.includes(i),p=v.side?(shared?[0,-1.7,.15]:[(i%24-11.5)*.125,(7.5-Math.floor(i/24))*.125,.15]):[(i%8-3.5)*.34,(1.5-Math.floor(i/8))*.5,.15];instance(d.experts,i,p,shared?[2.8,.12,.22]:v.side?[.102,.102,yes?.23:.075]:[.23,.28,.28],yes?col:palette.idle)}finish(d.experts);d.routes.forEach((r,i)=>{const id=v.side?(i===6?384:active[i]):i,p=v.side?(id===384?[0,-1.7,.2]:[(id%24-11.5)*.125,(7.5-Math.floor(id/24))*.125,.2]):[(id%8-3.5)*.34,(1.5-Math.floor(id/8))*.5,.2];r.geometry.setFromPoints([new THREE.Vector3(0,1.8,.3),new THREE.Vector3(...p)]);r.material.opacity=zoom*.45*otherFade});d.metric.el.innerHTML=v.side?'6 of 384<small>routed experts + 1 shared</small>':'2,048 hidden units<small>dense computation · ReLU</small>';
 }else if(d.vector){d.metric.el.innerHTML=(v.side?'5,120':'512')+'<small>dimensions per token</small>';}else if(d.streams){d.metric.el.innerHTML=v.side?'4 streams<small>Single-Pass mHC</small>':'Add & Norm<small>one residual stream · post-LayerNorm</small>';}
 else if(d.banks){d.metric.point.set(n.x,n.y+(v.side?1.15:2.5),2.8);const c=compute(state.n),growth=Math.cbrt(state.n/1048576),count=v.side?4:6;for(let i=0;i<count;i++){const ratio=v.side?Math.cbrt((i===3?356:178)/2048):1,k=growth*ratio;instance(d.banks,i,[(i-(count-1)/2)*.55,-1.2+1.5*k,.4],[.37*k,3*k,1.2*k],col)}finish(d.banks);d.metric.el.innerHTML=bytes(v.side?c.newCache:c.oldCache)+'<small>'+ (v.side?'global KV · 890 B/token':'cross-attention KV · assumed BF16')+'</small>';}
 else if(d.patches){const t=n.id==='vision_encoder'?0:(Math.sin(state.time*.65)+1)/2;for(let i=0;i<81;i++){const r=Math.floor(i/9),c=i%9,xx=(c-4)*.30*(1-t)+(Math.floor(c/3)-1)*.92*t+(c%3-1)*.055*t,yy=(r-4)*.30*(1-t)+(Math.floor(r/3)-1)*.92*t+(r%3-1)*.055*t;instance(d.patches,i,[xx,yy,.4+t*.5],[.18*(1-t*.55),.18*(1-t*.55),.14],palette.purple)}finish(d.patches);d.metric.el.innerHTML=n.id==='vision_encoder'?'32 layers<small>14-pixel input patches</small>':'9 patches → 1 position<small>2-layer vision projector</small>';}
 else if(d.slots){for(let i=0;i<48;i++){const row=Math.floor(i/8),y=(2.5-row)*.38+(row<3?.15:-.15),x=(i%8-3.5)*.34;instance(d.slots,i,[x,y,.15],[.23,.24,.17],palette.purple);instance(d.lookups,i,[x,y+(((step*7+i*11)%13)/12-.5)*.15,.26],[.2,.027,.04],col);}finish(d.slots);finish(d.lookups);d.metric.el.innerHTML='196B · 2 modules<small>layers 1 & 14 · 24 hash tables per module</small>';}
 else if(d.drafts){const t=(state.time*.3)%1,positions=[];for(let i=0;i<5;i++){const accepted=i<3,p=[(i-2)*.60,t<.6?1.15-t/.6*2.35:accepted?-1.2-(t-.6)*3:-1.2,.65];positions.push(new THREE.Vector3(...p));instance(d.drafts,i,p,!accepted&&t>.9?[0,0,0]:[.37,.32,.4],t<.6?palette.blue:accepted?col:palette.red)}finish(d.drafts);d.markov.geometry.setFromPoints(positions);d.markov.material.opacity=t<.6?zoom*.5*otherFade:0;d.metric.el.innerHTML='5 draft positions<small>optional · 3 blocks · target verification</small>';}

 else if(d.pool){const candidates=Math.min(state.n,16384),selected=Math.min(state.n,512),activeGroups=Math.round(64*selected/candidates);for(let i=0;i<64;i++){const yes=(i*17+step*7)%64<activeGroups;instance(d.pool,i,[(i%8-3.5)*.32,(3.5-Math.floor(i/8))*.32,.2],[.22,.24,yes?.5:.12],yes?palette.blue:palette.idle)}finish(d.pool);d.metric.el.innerHTML=`${candidates.toLocaleString('en-US')} → ${selected}<small>candidate entries → selected entries</small>`;}

 else if(d.distribution){const logits=toyLogits(step),probabilities=softmax(logits),linear=n.id==='linear';for(let i=0;i<16;i++){const value=linear?logits[i]*.42:probabilities[i]*4,h=Math.max(.025,Math.abs(value)),y=linear?value/2:-1.05+h/2;instance(d.distribution,i,[(i-7.5)*.18,y,.25],[.12,h,.24],linear&&value<0?palette.purple:col)}finish(d.distribution);d.metric.el.innerHTML=linear?'Linear projection<small>512 hidden dimensions → vocabulary logits</small>':n.id==='target_head'?'Vocabulary head<small>129,280 vocabulary entries</small>':'Next-token probabilities<small>softmax · normalized illustrative values</small>';}

 else if(n.part==='position')d.metric.el.innerHTML='Sinusoidal positions<small>added to token embeddings</small>';
 else if(n.part==='hidden')d.metric.el.innerHTML='Final encoder states<small>decoder global KV is projected from these states</small>';
 else d.metric.el.innerHTML=n.label.replaceAll('\n',' ');
...
function updateStoryTransport(position=storyPosition(story.clock.time)){
 const label=position.complete?'Replay story':story.clock.playing?'Pause story':'Resume story';
 ...
 const nextLabel=position.index===chapters.length-1?'Finish story':'Next chapter';$('#story-next').setAttribute('aria-label',nextLabel);$('#story-next').title=nextLabel;
...
function startStory(){
 ...
 $('#story-toggle').setAttribute('aria-pressed','true');$('#story-toggle').setAttribute('aria-label','Exit story and explore');$('#story-toggle-label').textContent='Explore';$('#story-duration').hidden=true;
 story.clock.seek(0,performance.now());story.clock.play(performance.now());resize();applyStoryCue(storyPosition(0));updateStoryTransport();
}
function stopStory(){
 if(!story.active){home();return;}story.clock.pause(performance.now());story.active=false;story.blend=null;story.resumeAfterDialog=false;state.n=story.savedContext;for(const v of views){v.controls.enabled=true;v.surface.style.pointerEvents='';v.storyWeights=null;}
 ...
 $('#story-panel').hidden=true;$('#explore-controls').hidden=false;$('#story-toggle').setAttribute('aria-pressed','false');$('#story-toggle').setAttribute('aria-label','Start the one-minute story');$('#story-toggle-label').textContent='Story mode';$('#story-duration').hidden=false;
 $('#context').value=Math.log2(state.n);$('#context-value').textContent=format(state.n)+' tokens';resize();home();
}
function pauseStory(){if(!story.active)return;story.clock.pause(performance.now());updateStoryTransport();}
function interruptStory(){if(story.active)pauseStory();}
function toggleStoryPlayback(){if(!story.active)return;if(story.clock.playing)pauseStory();else{const replay=story.clock.time>=storyDuration;if(replay)blendStoryCamera();story.clock.play(performance.now());if(replay)applyStoryCue(storyPosition(0));updateStoryTransport();}}
function seekStory(time){blendStoryCamera();const p=story.clock.seek(time,performance.now());applyStoryCue(p);updateStoryTransport(p);}
...
story.chapterButtons=chapters.map((chapter,i)=>{const button=document.createElement('button');button.setAttribute('aria-label',`Chapter ${i+1}: ${chapter.name}`);button.title=chapter.name;button.onclick=()=>seekStory(chapter.start);$('#story-chapters').append(button);return button;});
$('#story-exit').onclick=stopStory;$('#story-toggle').disabled=true;$('#story-toggle').onclick=()=>story.active?stopStory():startStory();$('#story-play').innerHTML=icon('pause');$('#story-play').onclick=toggleStoryPlayback;$('#story-previous').onclick=previousChapter;$('#story-next').onclick=nextChapter;
...
function updatePlay(){$('#play').innerHTML=icon(state.playing?'pause':'play');$('#play').setAttribute('aria-label',state.playing?'Pause token flow':'Play token flow');$('#play').title=state.playing?'Pause':'Play';}
$('#play').onclick=()=>{state.playing=!state.playing;updatePlay()};$('#context').oninput=e=>{state.n=2**+e.target.value;$('#context-value').textContent=format(state.n)+' tokens'};$('#pace').onchange=e=>state.pace=+e.target.value;
...
document.addEventListener('visibilitychange',()=>story.clock.resetVisibility());
$('#source-content').innerHTML=sourceHTML;$('#sources').onclick=()=>{story.resumeAfterDialog=story.active&&story.clock.playing;if(story.resumeAfterDialog)pauseStory();$('#source-dialog').showModal();};$('#close-sources').onclick=()=>$('#source-dialog').close();$('#source-dialog').addEventListener('close',()=>{if(story.resumeAfterDialog&&story.active){story.clock.play(performance.now());updateStoryTransport();}story.resumeAfterDialog=false;});
try{const loader=new GLTFLoader(),[assets,data]=await Promise.all([Promise.all(['spatial-original','spatial-deepseek','detail_kit'].map(n=>loader.loadAsync('./assets/'+n+'.glb?review=6'))),fetch('./assets/spatial-data.json?review=6').then(r=>r.json())]);kit=assets[2].scene;cubeGeometry=kit.getObjectByName('cache_unit').geometry;tokenGeometry=kit.getObjectByName('token_unit').geometry;
 for(const [i,v] of views.entries()){v.root=assets[i].scene;v.data=data[i];v.world.add(v.root);v.wires=[];v.root.traverse(o=>{if(o.isMesh){if(/^(wire_|arrow_|repeat_)/.test(o.name))v.wires.push(o);o.material=o.material.clone();o.material.transparent=true;o.material.opacity=o.name.startsWith('repeat_')?.2:1;o.material.depthWrite=true;o.userData.initialOpacity=o.material.opacity}});for(const n of v.data.nodes){if(n.part&&(n.kind==='box'||['kv_label','residual_input'].includes(n.id)||n.kind==='position'||n.part==='head'))v.components.push(makeInterior(v,n));else if(n.label)addLabel(v,n.id.endsWith('position_label')?'Position':n.label.replaceAll('\n',' '),[n.x,n.y,1.5],n.part?'component':'small',n.part?()=>focus(v,n):null)}for(const f of v.data.frames)addLabel(v,f.label,[f.tx,f.ty,-.2],'small');v.flow=new THREE.InstancedMesh(tokenGeometry,new THREE.MeshBasicMaterial({color:0xffffff,transparent:true,toneMapped:false}),1024);v.flow.frustumCulled=false;v.flow.renderOrder=4;v.scene.add(v.flow)}state.ready=true;$('#loading').hidden=true;$('#story-toggle').disabled=false;resize();updatePlay();
}catch(e){console.error(e);$('#loading').textContent='Unable to open the architectures. Please reload.';state.error=String(e)}
let before=performance.now();function frame(now){const dt=Math.max(0,Math.min(.08,(now-before)/1000));before=now;if(state.ready){if(story.active){const p=story.clock.tick(now,!document.hidden);if(story.cueKey!==`${p.index}:${p.cueIndex}`)applyStoryCue(p);updateStoryCamera(now);updateStoryTransport(p);}if((story.active?story.clock.playing:state.playing)&&!document.hidden)state.time+=dt*(story.active?1:state.pace);
 ...
 for(const v of views){findHot(v);flow(v);v.components.forEach(d=>tickInterior(v,d));const fade=1-(story.active?(v.storyFocus??0)*.93:(v.hot?.zoom??0)*.72);v.wires.forEach(m=>m.material.opacity=m.userData.initialOpacity*fade);v.flow.material.opacity=fade;if(!story.active)v.controls.update()}const current=views[state.era],active=current.hot;$('#selection').hidden=!(active&&active.zoom>.55);if(active&&active.zoom>.55)$('#selection-name').textContent=(active.n.branch?active.n.branch[0].toUpperCase()+active.n.branch.slice(1)+' · ':'')+(active.n.label.replaceAll('\n',' ')||'Position encoding');updateLabels();renderer.setScissorTest(false);renderer.clear();renderer.setScissorTest(true);for(const v of views){const vp=viewport(v);if(!vp.show)continue;renderer.setViewport(vp.x,0,vp.w,height);renderer.setScissor(vp.x,0,vp.w,height);renderer.render(v.scene,v.camera)}renderer.setScissorTest(false)}requestAnimationFrame(frame)}requestAnimationFrame(frame);
